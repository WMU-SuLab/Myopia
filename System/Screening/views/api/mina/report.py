...
from Screening.utils.schemes.report import UserReportSearchForm
from Screening.viewModels.project import search_projects, generate_user_report_data
...
def get_user_report_data(request):
    ...
    if not report_data.get('report_file_url', None):
        return Response(
            InsufficientPreconditions(data=report_data, msg='no informed consent', chinese_msg='没有填写知情同意书'))
...
def get_user_report_pdf_file(request):
    ...
    if project.report_file_url:
        if is_obj_file_path(project.report_file_url):
            return redirect(obj_sign_url(project.report_file_url, 60))
        ...
        report_str = render_to_string(
            template_name='Screening/report/single.html',
